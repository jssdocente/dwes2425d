...
Lo visto anteriormente, está pensado más para código PHP puro. Si queremos incluir condicionales en un documento HTML, lo haremos de la siguiente forma:
...
Al igual que los condicionales, los bucles también se pueden incluir en un documento HTML, evitando el uso de echo para mostrar el contenido. Por ejemplo:
...
También se pueden mezclar condicionales y bucles en un mismo documento HTML, como en el siguiente ejemplo, donde se muestran las tablas de multiplicar en una lista desordenada, solo para los números pares:
...
!!! danger "Cuidado con la asignación"
    Si hacemos esto `$nombres_copia = [$nombres]`, no estamos copiando el array, sino que estamos creando un array con un único elemento que es el array original. Es decir, anidando un array en otro. Para copiarlo, hemos de hacer `$nombres_copia = $nombres`.

**Agregar elementos a un Array**

Los elementos se pueden agregar al final, al principio, o en una posición determinada.
...
- `array_pop($frutas);` //Borra el último elemento
...
Unir matrices es el concepto de combinar dos o más matrices en una sola, y se puede realizar de varias formas:

- `array_merge($array1, $array2);` //Combina dos o más matrices
...
El operador Spread (`...`) se utiliza para descomponer un array en sus elementos. Se puede utilizar para pasar los elementos de un array a una función (variadics), o para unir arrays, que es lo que vamos a ver en este ejemplo:
...
> Este operador es una forma mucho más clara de copiar un Array, se entiende mejor que la asignación directa. `$copia = $frutas`.
...
También se pueden definir arrays bidimensionales de forma más directa:
...
!!! danger "Cuidado con la complejidad"
    ...
    En el tema de [*Programación Orientada a Objetos*](03phpoo.md) veremos cómo crear clases y objetos en PHP.
...
- Un formulario se compone de una serie de campos de entrada, etiquetas y botones de envío. 
- Los campos de entrada pueden ser de varios tipos: texto, contraseña, botones de radio, casillas de verificación, menús desplegables, etc. - Cada campo de entrada se identifica mediante un nombre y un valor. 
- Cuando el usuario envía el formulario, los datos introducidos en los campos de entrada se envían al servidor web, donde se procesan con un script PHP.

Aquí nos centraremos en cómo procesar los datos no en cómo se diseñan.

En un formulario existen varios conceptos importantes:

- **Action**: es la URL a la que se enviarán los datos del formulario. Si no se especifica, se enviarán a la misma página.
- **Method**: es el método de envío de los datos. Puede ser `GET` o `POST`. En el caso de `GET`, los datos se envían en la URL, mientras que en el caso de `POST`, los datos se envían en el cuerpo de la petición.
- **Input**: es el campo de entrada de datos. Puede ser de varios tipos: texto, contraseña, botones de radio, casillas de verificación, menús desplegables, etc.
- **Label**: es la etiqueta que acompaña al campo de entrada. Sirve para indicar al usuario qué tipo de información debe introducir en el campo.
...
Si es una peetición tipo `GET` los datos se envían via URL con el formato `var1=valor1&var2=valor2…`. Por ejemplo: `ejemplo.php?nombre=Bruce+apellido1=Wayne`, donde cada parámetro se separa con `&` y el nombre y el valor con `=`, y el nobre del parámetro es igual al valor del atributo `name` del campo de entrada.

!!! tip "Codificación de URL"
    Los espacios en blanco se codifican como `+` o `%20`, y los caracteres especiales se codifican como `%xx`, donde `xx` es el código ASCII del carácter.
...
Los datos se recogen a través de variables superglobales, en este caso `$_GET` y `$_POST` (dependiendo del método de envío).
